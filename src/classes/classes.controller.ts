...
export class ClassesController {
  ...
  async adminAssignTrainer(
    ...
    const updated = await this.classesService.adminAssignTrainer(
      id,
      dto.trainerId,
    );
  ...
  async assignMeAsTrainer(
    ...
    const updated = await this.classesService.assignTrainerToClass(
      id,
      user.userId,
    );
    return { ok: true, data: updated };
  ...
  // +++ NUEVO: desasignarse como entrenador (trainer) +++
  @Patch(":id/unassign-me")
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles("trainer")
  @ApiBearerAuth()
  @ApiParam({ name: "id", format: "uuid" })
  ...
  async unassignMeAsTrainer(
    @GetUser() user: { userId: string },
    @Param("id", new ParseUUIDPipe()) id: string,
  ) {
    const updated = await this.classesService.unassignTrainerFromClass(
      id,
      user.userId,
    );
    return { ok: true, data: updated };
