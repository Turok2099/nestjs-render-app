...
// Simular dos imágenes de prueba diferentes
...
async function uploadExerciseWithImage(token, exerciseName, imageBuffer, imageName) {
  try {
    ...
    const tempImagePath = path.join(__dirname, `temp-${imageName}.jpg`);
    ...
    console.log(`📤 Enviando solicitud con imagen: ${imageName}`);
...
async function uploadClassWithImage(token, className, imageBuffer, imageName) {
  try {
    ...
    // Crear archivo temporal
    const tempImagePath = path.join(__dirname, `temp-class-${imageName}.jpg`);
    fs.writeFileSync(tempImagePath, imageBuffer);
    
    formData.append("image", fs.createReadStream(tempImagePath), {
      filename: `class-${imageName}`,
      contentType: "image/jpeg",
    });
    
    console.log(`📤 Enviando solicitud con imagen: class-${imageName}`);
...
async function runTwoImagesTest() {
  console.log("🧪 === TEST DE SUBIDA DE DOS IMÁGENES ===\n");
  ...
  const exercise1 = await uploadExerciseWithImage(token, "Ejercicio Test 1", testImageBuffer1, "test1");
  
  console.log("\n🎓 === SUBIENDO PRIMERA CLASE ===");
  const class1 = await uploadClassWithImage(token, "Clase Test 1", testImageBuffer2, "test1");
  
  console.log("\n🏋️ === SUBIENDO SEGUNDO EJERCICIO ===");
  const exercise2 = await uploadExerciseWithImage(token, "Ejercicio Test 2", testImageBuffer2, "test2");
  
  console.log("\n🎓 === SUBIENDO SEGUNDA CLASE ===");
  const class2 = await uploadClassWithImage(token, "Clase Test 2", testImageBuffer1, "test2");
...
runTwoImagesTest().catch(console.error);
